...
## Task:
我提供了一场预测失败的冷门（或赢球输盘）比赛数据，请你进行深度复盘分析。你需要重点关注：为什么当时模型没有识别到这个冷门？是陷入了基本面陷阱、战意陷阱、还是被盘口异常诱导了？
...
## Output Requirement:
请用两段话进行极其尖锐的回复：
1. **冷门致死原因剖析**：一针见血地指出当时预测逻辑的漏洞（例如：盲信名牌战意导致中了机构的退盘诱导、低估了弱队的防守韧性导致深盘打不穿、无视了主场平手盘的危险信号等），请直接指出是哪种特定的“冷门基因”发作，控制在 80 字以内。
2. **防冷门模型优化建议**：针对这类特定的冷门特征，给出现在预测模型的一条具体避坑建议或规则修改建议（如：下次遇到XX情况，必须无脑防平）。
"""
            try:
